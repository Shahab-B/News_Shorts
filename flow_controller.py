...
def run_pipeline(article_text):
    ...
    raw_script = script_writer.run(scenes, script_writer_prompt)

    # Postprocess: replace names with descriptors in scene descriptions only
    cleaned_script = auto_replace_name_leaks(raw_script)
